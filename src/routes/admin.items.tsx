...
function ItemsAdmin() {
  ...
  const filtered = useMemo(() => {
    ...
    return state.items
      ...
      .sort((a, b) => a.categoryId.localeCompare(b.categoryId) || a.sort_order - b.sort_order);
  }, [state.items, filterCat, q]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 rounded-xl border border-border bg-card p-3">
        <div className="relative min-w-[200px] flex-1">
          ...
          <Input placeholder="Поиск по названию…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
        </div>
        <Select value={filterCat} onValueChange={setFilterCat}>
          <SelectTrigger className="w-[220px]"><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Все категории</SelectItem>
            {cats.map((c) => <SelectItem key={c.id} value={c.id}>{c.name_ru}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>

      <div className="overflow-hidden rounded-xl border border-border bg-card">
        <div className="hidden grid-cols-[64px_minmax(0,2fr)_minmax(0,1fr)_120px_auto_auto] gap-3 border-b border-border bg-secondary/40 px-4 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground md:grid">
          <span></span><span>Название</span><span>Категория</span><span>Цена</span><span>Статус</span><span></span>
        </div>
        {filtered.length === 0 && (
        ...
        {filtered.map((i) => (
          <div
            key={i.id}
            className="grid grid-cols-[56px_minmax(0,1fr)_auto] items-center gap-3 border-b border-border px-3 py-3 last:border-b-0 md:grid-cols-[64px_minmax(0,2fr)_minmax(0,1fr)_120px_auto_auto] md:px-4"
          >
            ...
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex flex-wrap gap-1 md:hidden">
                <span className="text-xs text-muted-foreground">{catName(i.categoryId)} · {formatPrice(i.price)}</span>
              </div>
            ...
            <span className="hidden truncate text-sm text-muted-foreground md:block">{catName(i.categoryId)}</span>
            <span className="hidden text-sm font-semibold md:block">{formatPrice(i.price)}</span>
            <span className="hidden text-xs text-muted-foreground md:block">{i.weight ?? "—"}</span>
            <div className="flex items-center gap-1 justify-self-end">
              ...
              <DeleteConfirm
                ...
                onConfirm={() => menuStore.deleteItem(i.id)}
                trigger={
