      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        {cats.length === 0 && (
          <p className="px-4 py-10 text-center text-sm text-muted-foreground">
            Категорий пока нет. Создайте первую категорию для меню.
          </p>
        )}
        {cats.map((c, idx) => {
          const dishesCount = countFor(c.id);
          const canDelete = dishesCount === 0;
          return (
            <div
              key={c.id}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-muted-foreground">
                  {dishesCount} блюд · {c.is_active ? "Активна" : "Скрыта"}
                </div>
                {!canDelete && (
                  <div className="mt-1 text-xs text-muted-foreground">
                    Чтобы удалить категорию, сначала перенесите или удалите блюда.
                  </div>
                )}
              </div>
              <Button
              ...
              {canDelete ? (
                ...
                <Button
                  ...
                  variant="ghost"
                  disabled
                  aria-label="Нельзя удалить категорию с блюдами"
                  title="Сначала перенесите или удалите блюда"
                >
